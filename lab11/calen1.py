...
def add_note_to_date(date):
    ...
    if note:
        ...
        day_label = Label(root, text=date, width=4, height=2, font='Arial 16 bold', fg='blue')
...
def fill():
    ...
    for n in range(month_days):
        if year == now.year and month == now.month and n + 1 == now.day:
            day_number = n + 1
            day_label = Label(root, text=day_number, width=4, height=2, font='Arial 16 bold', bg='green', fg='white')
            ...
            day_number = n + 1
            day_label = Label(root, text=day_number, width=4, height=2, font='Arial 16 bold')
            day_label.grid(row=2 + (n + week_day) // 7, column=(n + week_day) % 7, sticky=NSEW)
            day_label.bind("<Button-1>", lambda event, day=day_number: add_note_to_date(day))
